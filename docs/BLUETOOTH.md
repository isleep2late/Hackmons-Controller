...
- **GameCube face buttons.** The bits are read positionally like the Pro Controller's, as SDL 3.4's `HandleGameCubeState` does (the "A" bit is `east`, the "B" bit `south`), so the pad reads the same over BLE, over USB and in the Android app; which physical button sets which bit still has to be confirmed.
